...
play_page = cv2.imread('../Resources/Menu/Play_page1.png', cv2.IMREAD_UNCHANGED)
play_page = cv2.resize(play_page, (640, 480))  # ปรับขนาดให้ตรงกับเฟรม
opacity = 0.8  # กำหนดค่า opacity
currWindow = im1
...
while True:
    ...
    if inFrame == 1:
        ...
        # เพิ่มโค้ดที่นี่เพื่อแสดงภาพ Play_page1.png เมื่อไม่เห็นผู้เล่น
        overlay = play_page.copy()
        cv2.addWeighted(overlay, opacity, frm, 1 - opacity, 0, frm)
