...
while True:
    ...
    # Shop state
        ...
        if game_over:
            ...
            # Display the appropriate image on the left side of the screen
            img_height, img_width = side_image.shape[:2]
            # Check if side_image has 4 channels (RGBA)
            # Ensure the side_image has transparency and use cv2.addWeighted to blend it
            if side_image.shape[2] == 4:  # Check for alpha channel
                ...
                for c in range(3):  # For each color channel
                    frame[50:50 + img_height, 50:50 + img_width, c] = (
                            mask * overlay[:, :, c] +
                            (1 - mask) * frame[50:50 + img_height, 50:50 + img_width, c]
                    )
            else:
                frame[50:50 + img_height, 50:50 + img_width] = side_image
